...
(defn add "Add a key and value to the BST."
  [bst nu-key nu-val]
  (let [nu-size (atom (size bst))]
    (let [root (add-aux (:data bst) nu-key nu-val nu-size)]
      (BST. root @nu-size))))

(defn add-aux "add without the wrapper class. Only returns root node" 
  [node nu-key nu-val nu-size]
  (cond (nil? (:key node)) (do (swap! nu-size inc) (make-node nu-key nu-val)) ; root node
        (= nu-key (:key node)) (make-node (:left node) nu-key nu-val (:right node)) ; replace value
        ; nu-key is left of node
        (> 0 (compare nu-key (:key node))) (if (nil? (:left node))  
                                             (do
                                               (swap! nu-size inc) ; inc size if adding node
                                               (make-node (make-node nu-key nu-val) (:key node) (:value node) (:right node)))
                                             (make-node (add-aux (:left node) nu-key nu-val nu-size) (:key node) (:value node) (:right node)))
        :else (if (nil? (:right node))
                (do
                  (swap! nu-size inc) ;inc size if adding node
                  (make-node (:left node) (:key node) (:value node) (make-node nu-key nu-val))) 
                (make-node (:left node) (:key node) (:value node) (add-aux (:right node) nu-key nu-val nu-size)))))
;; # Find
;;
