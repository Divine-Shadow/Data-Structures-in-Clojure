...
(defn dequeue
  ...
  [queue]
  (cond (and (empty? (:front queue)) (empty? (:back queue))) (make-queue)
             (empty? (:front queue)) (dequeue (Queue. nil (reverse (:back queue)) (:size queue)))
             (= (rest (:front queue)) '()) (Queue. (:back queue) nil (-> queue :size dec))
