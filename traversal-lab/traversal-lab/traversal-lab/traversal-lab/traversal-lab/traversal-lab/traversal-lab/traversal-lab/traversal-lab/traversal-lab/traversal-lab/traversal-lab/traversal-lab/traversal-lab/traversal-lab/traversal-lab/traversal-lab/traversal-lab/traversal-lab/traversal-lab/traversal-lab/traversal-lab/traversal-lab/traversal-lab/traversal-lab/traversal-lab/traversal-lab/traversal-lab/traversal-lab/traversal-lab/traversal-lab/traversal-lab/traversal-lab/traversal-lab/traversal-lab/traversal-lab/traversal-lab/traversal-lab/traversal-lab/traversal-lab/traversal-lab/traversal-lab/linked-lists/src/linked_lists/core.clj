...
(defn sorted-insert
  ...
  [elt xx]
  (cond (empty? xx) (Cons. elt nil)
        (>= elt (:car xx)) (Cons. (:car xx) (sorted-insert elt (:cdr xx)))
        :else (Cons. elt xx)))
...
(defn delete
  ...
  [elt xx]
  (cond (empty? xx) nil
        (= elt (:car xx)) (:cdr xx);(Cons. (:car (:cdr xx)) (:cdr (:cdr xx))) ;return :cdr xx (change made was take out the cond)
...
(defn efficient-delete
  ...
  *original* xx instead of a copy.  It is acceptable to prescan the
  list."
  [elt xx]
  (cond (not (search elt xx)) xx
        :else (cond (empty? xx) xx
                   (= elt (:car xx)) (:cdr xx)
                   :else (Cons. (:car xx) (delete elt (:cdr xx))))))
  ...
  ;(cond (empty? xx) nil
   ;     (not (= elt (:car xx))) xx ;need a do while loop
    ;    (= elt (:car xx)) (:cdr xx)
