...
(defn insert-last "Insert an element into the back of a dlist."
  [xx elt]
  (let [nu-node (dnode (d-sentinel xx) elt (-> xx d-sentinel d-prev))]
...
Uses sentinels, so it's very short."
  [xx victim]
  (swap-d-size! xx (- 1 (d-size xx)))
  (cond (and (d-prev victim) (d-next victim)) ; middle node
        (do (reset-d-next! (d-prev victim) (d-next victim))
            (reset-d-prev! (d-next victim) (d-prev victim)))
        (d-prev victim) ; last node
        (do (reset-d-next! (d-prev victim) (d-next victim))
            (setLast! xx (d-prev victim)))
        (d-next victim) ; first node
        (do (reset-d-prev! (d-next victim) (d-prev victim))
            (setFirst! xx (d-next victim)))
        :else ; only node
        (do (setFirst! xx nil)
            (setLast! xx nil))))
