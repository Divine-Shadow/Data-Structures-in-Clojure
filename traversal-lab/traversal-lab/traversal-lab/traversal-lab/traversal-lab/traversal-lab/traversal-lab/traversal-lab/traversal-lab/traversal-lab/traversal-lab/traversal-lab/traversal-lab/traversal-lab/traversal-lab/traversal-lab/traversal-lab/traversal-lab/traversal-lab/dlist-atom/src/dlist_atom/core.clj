...
(declare insert-sorted-aux)
(defn insert-sorted "Insert an element in sorted order"
  [xx elt]
(cond (= (d-size xx) 0) (insert-front xx elt)
      (> elt (-> xx d-sentinel d-prev d-data)) (insert-last xx elt)
      :else (insert-sorted-aux (-> xx d-sentinel d-next) elt xx))
)

(defn insert-sorted-aux [current elt xx]
(let [nu-node (dnode nil elt nil)]
(cond (identical? current (d-sentinel xx)) nil
(< elt (d-data current))  (do (reset-d-next! (d-prev current) nu-node)
			      (reset-d-next! nu-node current)
		              (reset-d-prev! nu-node (d-prev current))
		      	      (reset-d-prev! current nu-node)
			      (swap-d-size! xx inc))
:else (insert-sorted-aux (d-next current) elt xx)))

)

(declare index-forward-aux)
...
I.e., if it was the first element, return 0; second, return 1..."
  [xx elt]
(index-forward-aux (-> xx d-sentinel d-next) elt xx 0)

)

(defn index-forward-aux [current elt xx number] 
(cond (identical? current (d-sentinel xx)) nil
(= elt (d-data current)) number
(= number (- (d-size xx) 1)) nil
:else (index-forward-aux (d-next current) elt xx (+ number 1)))

)

(declare index-backward-aux)
...
I.e., if it was the last element, return -1; penultimate, return -2..."
  [xx elt]
(index-backward-aux (-> xx d-sentinel d-prev) elt xx -1) 

)

(defn index-backward-aux [current elt xx number]
(cond (identical? current (d-sentinel xx)) nil
(= elt (d-data current)) number
(= number (* (d-size xx) -1)) nil
:else (index-backward-aux (d-prev current) elt xx (- number 1)))

)

(declare list-to-dlist-aux)
(defn list-to-dlist "Given a Clojure list, return a DList with the same content."
  [xx]
(let [template (dlist)]  
(list-to-dlist-aux template xx))

)

(defn list-to-dlist-aux [template xx]
(if (empty? xx) template
(do (insert-last template (first xx))
    (list-to-dlist-aux template (rest xx))))
...
Uses sentinels, so it's very short."
  [xx victim]
(delete-aux (-> xx d-sentinel d-next) victim xx)
)

(defn delete-aux [current elt xx] 
(cond (identical? current (d-sentinel xx)) nil
(= elt (d-data current)) (do (reset-d-next! (d-prev current) (d-next current))
		         (reset-d-prev! (d-next current) (d-prev current))
			 (swap-d-size! xx dec))
:else (delete-aux (d-next current) elt xx))
...
(defn reverse "Reverse the doubly linked list in place.  No new DNode or DList records are created."
  ...
  (let [old-next (-> xx d-sentinel d-next)]
      (do (reset-d-next! (d-sentinel xx) (-> xx d-sentinel d-prev))
          (reset-d-prev! (d-sentinel xx) old-next)))

)

(defn reverse-aux [sen node]
(if (identical? node sen) nil
...
(defn show-dlist-reverse "Like show-dlist, but returns the items in reverse order."
  [xx]
  (show-dlist-reverse-aux (d-sentinel xx) (-> xx d-sentinel d-prev) (-> xx d-sentinel d-prev d-prev))

)

(defn show-dlist-reverse-aux [sen node runner]
(cond (identical? node sen) '()
      (identical? node runner) '(infinite-loop)
