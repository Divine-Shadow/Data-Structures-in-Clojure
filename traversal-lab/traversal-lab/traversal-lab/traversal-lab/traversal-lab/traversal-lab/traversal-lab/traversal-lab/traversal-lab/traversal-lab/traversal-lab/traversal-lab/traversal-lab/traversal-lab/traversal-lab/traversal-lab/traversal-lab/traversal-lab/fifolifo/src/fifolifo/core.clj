...
(defn pop
  ...
  (cond (empty? stk) nil
  (= (:size stk) 0) stk
  :else  (Stack. (next (:top stk)) (-> stk :size dec))
    ))
...
(defn top
  ...
  (first (:top stk)))
;(def s (make-stack))
;(top(push(pop(pop(push (push (push s 5) 10) 20)))10))
;; # Doubly-ended queues
;;
...
(defn chk-reverse [queue]
  (if (empty? (:front queue)) (Queue. nil (reverse (:back queue)) (:size queue)) queue)
  )
(defn make-queue
...
(defn dequeue
  ...
  [queue]
  (if (= (:size queue) 0) queue
   (let [qu (chk-reverse queue)]
   (Queue. (:back qu) (next (:front qu)) (dec (:size qu))))
  ))
...
;(def q (make-queue))
;(dequeue (enqueue (enqueue (enqueue (enqueue q 5) 10) 15) 20))
