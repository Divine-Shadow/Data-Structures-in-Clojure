...
(defn insert-sorted "Insert an element in sorted order"
  [xx elt]
  (insert-sorted-aux xx (-> xx d-sentinel d-next) elt))

(defn insert-sorted-aux
  [dlist node elt]
  (cond (identical? (d-sentinel dlist) node) (insert-last dlist elt) ; If EOList reached elt belongs at end
        (< elt (d-data node)) (let [nu-node (dnode (d-prev node) elt node)]
                                (reset-d-next! (d-prev node) nu-node)
                                (reset-d-prev! node nu-node)
                                (swap! (:size dlist) inc))
        :else (insert-sorted-aux dlist (d-next node) elt)))
...
I.e., if it was the first element, return 0; second, return 1..."
  [xx elt]
  (index-forward-aux (d-sentinel xx) (-> xx d-sentinel d-next) elt 0))

(defn index-forward-aux [sen node elt index]
  (cond (identical? sen node) nil
        (= (d-data node) elt) index
        :else (index-forward-aux sen (d-next node) elt (+ index 1))))
...
I.e., if it was the last element, return -1; penultimate, return -2..."
  [xx elt]
  (index-backward-aux (d-sentinel xx) (-> xx d-sentinel d-prev) elt -1))
  
(defn index-backward-aux [sen node elt index]
  (cond (identical? sen node) nil
        (= (d-data node) elt) index
        :else (index-backward-aux sen (d-prev node) elt (- index 1))))
...
(defn list-to-dlist "Given a Clojure list, return a DList with the same content."
  ...
  (let [dl (dlist)]
    (do
      (list-to-dlist-aux xx dl)
      dl)))

(defn list-to-dlist-aux [xx dl]
  (cond (empty? xx) (d-sentinel dl) ; if end of list return nil
        :else (do
                (insert-last dl (first xx))
                (list-to-dlist-aux (rest xx) dl))))
...
Uses sentinels, so it's very short."
  [xx victim]
  (delete-aux xx (d-sentinel xx) (-> xx d-sentinel d-next) victim))

(defn delete-aux [dlist sen node victim]
  (cond (identical? sen node) nil
        (= (d-data node) victim) (do
                                   (reset-d-next! (d-prev node) (d-next node))
                                   (reset-d-prev! (d-next node) (d-prev node))
                                   (swap! (:size dlist) dec))
        :else (delete-aux dlist sen (d-next node) victim)))

(declare reverse-aux)
(defn reverse "Reverse the doubly linked list in place.  No new DNode or DList records are created."
  [xx]
  (reverse-aux (-> xx d-sentinel d-next) (d-sentinel xx)))

(defn reverse-aux [node sen]
  (let [old-next (d-next node)]
    (reset-d-next! node (d-prev node))
    (reset-d-prev! node old-next)
    (if (identical? node sen) nil (reverse-aux old-next sen))))

(declare show-dlist-reverse-aux)
(defn show-dlist-reverse "Like show-dlist, but returns the items in reverse order. Will most likely break if any nodes have a :prev of nil"
  [xx]
