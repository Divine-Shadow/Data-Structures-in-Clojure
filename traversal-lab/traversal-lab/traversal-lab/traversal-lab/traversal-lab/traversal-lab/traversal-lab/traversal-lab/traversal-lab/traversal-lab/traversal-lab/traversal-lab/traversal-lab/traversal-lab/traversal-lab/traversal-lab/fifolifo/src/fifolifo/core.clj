...
(defn dequeue
  ...
  (cond
    (= (:size queue) 0) queue
    (empty? (:front queue)) (Queue. nil (-> queue :back reverse rest) (-> queue :size dec))
    :else                   (Queue. (:back queue) (-> queue :front rest) (-> queue :size dec))))
...
(defn peek
  ...
  (cond
    (= (:size queue) 0) nil
    (empty? (:front queue)) (first (-> queue :back reverse))
