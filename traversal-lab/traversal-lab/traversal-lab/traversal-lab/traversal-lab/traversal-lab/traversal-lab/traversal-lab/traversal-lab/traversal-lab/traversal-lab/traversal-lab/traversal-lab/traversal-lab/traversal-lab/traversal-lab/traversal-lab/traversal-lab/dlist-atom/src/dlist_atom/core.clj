...
(defn insert-before-dnode [dn elt]
  (let [nu-node (dnode (-> dn d-prev) elt dn)]
    (reset-d-next! (-> dn d-prev) nu-node)
    (reset-d-prev! (-> nu-node d-next) nu-node)
    )
  )
(defn delete-dnode [dn]
    (reset-d-next! (-> dn d-prev) (-> dn d-next))
    (reset-d-prev! (-> dn d-next) (-> dn d-prev))
  )
(defn insert-sorted-aux [dn elt]
  (cond (= (-> dn d-data) 'sentinel) (insert-before-dnode dn elt)
        (< (-> dn d-data) elt) (insert-sorted-aux (-> dn d-next) elt)
        :else (insert-before-dnode dn elt)
   )
  )
(defn insert-sorted "Insert an element in sorted order"
  [xx elt]
  (insert-sorted-aux (-> xx d-sentinel d-next) elt)
  (swap-d-size! xx inc)
  )
(defn index-forward-aux [dn elt c]
  (cond (= (-> dn d-data) 'sentinel) nil
        (= (-> dn d-data) elt) c
        :else (index-forward-aux (-> dn d-next) elt (inc c))
  ))
...
(defn index-backward-aux [dn elt c]
  (cond (= (-> dn d-data) 'sentinel) nil
        (= (-> dn d-data) elt) c
        :else (index-backward-aux (-> dn d-prev) elt (dec c))
  ))
...
(defn list-to-dlist "Given a Clojure list, return a DList with the same content."
  [xx]
  (if (empty? xx) (dlist) (list-to-dlist-aux xx (dlist)))
  )
