...
(defn dequeue
  ...
  [queue]
  (cond (= 0 (:size queue)) (make-queue)
        (empty? (:front queue)) (let [x (rest(reverse (:back queue)))]
                (Queue. nil x (-> queue :size dec)))
...
(defn peek
  ...
  [queue]
  (cond (= true (empty? (:front queue))) 
        (if (= true (empty? (:back queue))) nil
