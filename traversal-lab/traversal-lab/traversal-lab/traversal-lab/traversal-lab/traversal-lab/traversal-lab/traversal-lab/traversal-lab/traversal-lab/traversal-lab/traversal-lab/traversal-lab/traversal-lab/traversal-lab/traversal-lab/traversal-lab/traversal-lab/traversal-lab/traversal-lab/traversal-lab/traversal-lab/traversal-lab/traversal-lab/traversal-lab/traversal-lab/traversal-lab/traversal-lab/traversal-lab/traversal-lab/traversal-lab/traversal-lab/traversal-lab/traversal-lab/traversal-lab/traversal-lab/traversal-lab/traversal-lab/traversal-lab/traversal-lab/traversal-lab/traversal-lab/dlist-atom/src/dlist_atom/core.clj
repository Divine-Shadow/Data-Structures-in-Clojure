...
(defn insert-front "Insert an element into the front of a dlist."
  ...
  (let [nu-node (dnode (d-sentinel xx) elt (-> xx d-sentinel d-next))]
    (do (reset-d-next! (d-sentinel xx) nu-node)
...
(defn insert-last "Insert an element into the back of a dlist."
  ...
  (let [nu-node (dnode (-> xx d-sentinel d-prev) elt (d-sentinel xx))]
    (do (reset-d-prev! (d-sentinel xx) nu-node)
...
(defn insert-sorted-aux
  [dlist node elt]
  (cond (identical? (d-sentinel dlist) node) (insert-last dlist elt)
...
I.e., if it was the first element, return 0; second, return 1..."
  [xx elt]
  (let [y (-> xx d-sentinel d-next)]
  (index-forward-aux y elt 0)))

(defn index-forward-aux [xx elt counter]
    (cond (= (d-data xx) 'sentinel) nil
          (= (d-data xx) elt) counter
          :else (index-forward-aux (d-next xx) elt (inc counter))))
...
I.e., if it was the last element, return -1; penultimate, return -2..."
  [xx elt]
  (let [z (-> xx d-sentinel d-prev)]
    (index-backward-aux z elt -1)))
(defn index-backward-aux
  [xx elt count]
  (cond (= (d-data xx) 'sentinel) nil
        (= (d-data xx) elt) count
        :else (index-backward-aux (d-prev xx) elt (dec count))))
...
(defn list-aux [yy y]
  (cond (nil? (first yy)) y
        :else (do (insert-last y (first yy))
                  (list-aux (rest yy) y))))
...
(defn reverse "Reverse the doubly linked list in place.  No new DNode or DList records are created."
  [xx]
  (reverse-aux (:sentinel xx) (-> xx d-sentinel d-next) xx)
  (let [temp (-> xx d-sentinel d-next)]
    (do (reset-d-next! (-> xx d-sentinel) (-> xx d-sentinel d-prev))
        (reset-d-prev! (-> xx d-sentinel) temp))))
(defn reverse-aux [sen node xx]
  (if (identical? node sen) nil
      (let [old-next (d-next node)]
        (do (reset-d-next! node (d-prev node))
            (reset-d-prev! node old-next)
            (reverse-aux sen (d-prev node) xx)))))

(declare show-dlist-reverse-aux)
(defn show-dlist-reverse "Return a list representation of the contents of a DList."
