...
(defn pop
"Remove an element from the top of the stack. Retrun the resulting stack"
 [stk]
 (if (empty? (:top stk) )
       (Stack. (rest stk) (-(:size stk) 1))))
;;(Defn  Return the resulting stack."
;; [stk]
;;nil)
...
(defn delete-from-last 
"deltes the last element from a list"
[list]
...
(if (empty? queue) queue
    (Queue. (delete-from-last [:back queue]) (rest (:front queue)) (-> queue :size))))
