...
(defn pop
  ...
  [stk]
  (let [p (Stack. (next (:top stk)) (-> stk :size dec))] p)
...
(defn dequeue
  ...
  [queue]
  (if (empty? (:front queue)) (Queue. (((:back queue) nil) (next (reverse (:back queue))) (-> queue :size dec))))
  (Queue. (next (:back queue)) (:first queue) (-> queue :size dec))
