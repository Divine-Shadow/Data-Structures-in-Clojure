...
(defn push
  ...
  [stk elt]
  (cond (empty? (:top stk))(Stack. (cons elt nil) 1)
    :else (Stack. (cons elt (:top stk)) (inc (stack-size stk)))))
...
(defn enqueue
  ...
  [queue elt]
  (cond (empty? (:back queue)) (Queue. (cons elt nil) (:front queue) (inc (:size queue)))
...
(defn rev
  ...
  [xx]
  (cond (empty? xx) nil 
    :else (cons (rev (rest xx)) (first xx))))

(defn dequeue
  "Remove an element from the back of the queue.  Just return the new queue."
  [queue]
  (cond (= (:size queue) 0) (Queue. nil nil 0)
  	(empty? (:front queue)) (Queue. nil (rest (rev (:back queue))) (dec (:size queue)))
