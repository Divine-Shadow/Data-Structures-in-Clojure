...
(describe "sorted insert"
          ...
          (it "should work with empty lists."
              (should= (Cons. 10 nil) (sorted-insert 10 nil)))
          (it "should work with lists that are sorted."
              (let [xx (Cons. 1 (Cons. 2 (Cons. 4 nil)))]
                (should= (Cons. 1 (Cons. 2 (Cons. 3 (Cons. 4 nil)))) (sorted-insert 3 xx))))
          (it "should work with lists that are not sorted."
              (let [xx (Cons. 3 (Cons. 1 (Cons. 4 (Cons. 9 nil))))]
                (should= (Cons. 2 (Cons. 3 (Cons. 1 (Cons. 4 (Cons. 9 nil))))) (sorted-insert 2 xx))))
          (it "should recycle memory."
              (let [xx (Cons. 1 (Cons. 5 (Cons. 6 (Cons. 7 nil))))]
                (should (identical? (:cdr xx) (:cdr (:cdr (sorted-insert 4 xx)))))))
          (it "should work with lists that are sorted until the middle."
...
(describe "delete-all"
          ...
          (it "should work with list if element is not in the list."
              (let [xx (Cons. 1 (Cons. 2 (Cons. 3 nil)))]
                (should= xx (delete-all 5 xx))))
)

(describe "efficient-delete"
          (it "it should work with empty lists."
