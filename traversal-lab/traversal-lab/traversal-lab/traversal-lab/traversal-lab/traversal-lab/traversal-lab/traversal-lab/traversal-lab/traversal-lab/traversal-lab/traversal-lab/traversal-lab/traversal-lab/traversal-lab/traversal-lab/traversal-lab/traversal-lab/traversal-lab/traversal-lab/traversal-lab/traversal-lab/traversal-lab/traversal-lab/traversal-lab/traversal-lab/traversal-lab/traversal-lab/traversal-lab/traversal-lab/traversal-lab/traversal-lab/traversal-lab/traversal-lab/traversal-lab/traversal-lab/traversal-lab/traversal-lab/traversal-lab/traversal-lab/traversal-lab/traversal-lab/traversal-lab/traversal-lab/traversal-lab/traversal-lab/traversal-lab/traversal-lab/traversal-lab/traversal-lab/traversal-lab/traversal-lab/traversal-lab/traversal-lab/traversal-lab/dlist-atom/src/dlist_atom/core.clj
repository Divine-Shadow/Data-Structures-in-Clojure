...
;; Helper function deletes the required node
;; Helper function to delete

(defn delete-node [xx node]
...
;; Helper function for index forward
;; Moves the counter forward as it traverses through the list

(defn index-f-h [dns elt counter]
   (cond (= 'sentinel (d-data dns)) nil
      (= elt (d-data dns)) counter
        :else (index-f-h (d-next dns) elt (inc counter))))
...
I.e., if it was the first element, return 0; second, return 1..."
   [xx elt]
    (index-f-h (-> xx d-sentinel d-next) elt 0))

;; Helper function for index backwards
;; Does same thing as helper for index forward, but negates the counter instead

(defn index-b-h [dns elt counter]
   (cond (= 'sentinel (d-data dns)) nil
...
I.e., if it was the last element, return -1; penultimate, return -2..."
    [xx elt] ; pass in 1 for counter
           (index-b-h (-> xx d-sentinel d-prev) elt 1))

;; Helper function for list-to-d-list
;; Converts to a list to a d-list

(defn ldl-h [dlist1 xx]
  (let [elt (first xx)]
       (if (nil? elt) dlist1
       (do
       (insert-last dlist1 elt)
       (ldl-h dlist1 (rest xx))))))
...
;; Helper function to delete
;; Helps to find the element that needs to be deleted

(defn D-search [dns elt]
   (cond (= 'sentinel (d-data dns)) nil
      (= elt (d-data dns)) dns
        :else (D-search (d-next dns) elt)))
...
Uses sentinels, so it's very short."
  [xx victim]
  (let [dead (D-search (-> xx d-sentinel d-next) victim)]
   (if (nil? dead) nil
   (do
   ...
   (swap! (:size xx) dec)
   (reset-d-prev! dead dead)))))

;; Helper function to reverse
;; Reverses each node

(defn rev-help [sent node]
  ...
  (rev-help sent (d-prev node))))))))

(defn reverse "Reverse the doubly linked list in place.  No new DNode or DList records are created."
  [xx]
  (rev-help (-> xx d-sentinel) (-> xx d-sentinel d-next)))

(declare sdlr-help)

;; Helper function to show-d-list-reverse

(defn sdlr-help "Run through the data of a Dlist, stopping when reaching the sentinel or
discovering an infintite loop.  The runner goes twice as fast through the list; if it reaches
node we know we have an infinite loop."
  [sen node runner]
  (cond (identical? sen node) '()
        (identical? node runner) '(infinite-loop)
        :else (cons (d-data node) (sdlr-help sen (d-prev node) (-> runner d-prev d-prev)))))


(defn show-dlist-reverse "Like show-dlist, but returns the items in reverse order."
  [xx]
  (show-dlist-aux (d-sentinel xx) (-> xx d-sentinel d-prev)
