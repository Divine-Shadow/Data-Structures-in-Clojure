...
(defn find-location [xx node elt]
  (cond (= 'sentinel (d-data node)) (d-sentinel xx)
        (> elt (d-data node)) node
        :else (find-location xx (d-next node) elt)))
...
(defn insert-sorted "Insert an element in sorted order"
  [xx elt]
  (if (= (d-size xx) 0) (insert-front xx elt)
      (let [loc (find-location xx (get-front xx) elt)
            nu-node (dnode (d-prev loc) elt loc)]
        (do (reset-d-prev! loc nu-node)
            (reset-d-next! (d-prev loc) nu-node)
            (swap-d-size! xx inc))))
)
