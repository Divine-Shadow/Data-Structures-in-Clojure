...
(defn search 
  ...
  [elt xx]
  (cond (empty? xx) false (= (:car xx) elt) true :else (search elt (:cdr xx))))
...
(defn delete
  ...
  [elt xx]
     (cond (empty? xx) nil (= elt (:car xx)) (:cdr xx) 
...
(defn delete-all 
  ...
  [elt xx]
   (cond (empty? xx) nil (= elt (:car xx)) (delete-all elt (:cdr xx)) 
...
(defn delete-scanner 
  [elt xx] 
   (cond (empty? xx) 0 (= (:car xx) elt) 1 :else (delete-scanner elt (:cdr xx))))
...
(defn efficient-delete
  ...
  *original* xx instead of a copy.  It is acceptable to prescan the
  list."
  [elt xx] 
  (cond (= (delete-scanner elt xx) 0) xx :else (delete elt xx)))
