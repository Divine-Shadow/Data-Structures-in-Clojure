...
(defn insert-front "Insert an element into the front of a dlist."
 ...
 (let [nu-node (dnode (d-sentinel xx) elt (-> xx d-sentinel d-next))]
   ...
   (reset-d-prev! (-> nu-node d-prev) nu-node)
   (reset-d-size! (d-size xx) (+ 1 (d-size xx))))
 )
...
(defn insert-last "Insert an element into the back of a dlist."
  ...
  (let [nu-node (dnode (-> xx d-sentinel d-prev) elt (d-sentinel xx))]
      ...
      (reset-d-next! (-> nu-node d-next) nu-node)
      (reset-d-size! (d-size xx) (+ 1 (d-size xx))))
  )
...
(defn insert-sorted "Insert an element in sorted order"
  ...
  (let [nu-node (dnode (d-sentinel xx) elt (d-sentinel xx))]
  	(cond (nil? (:data))
                (do
                  (+ 1 (d-size xx))
                  (insert-sorted (d-next (d-sentinel xx) xx) elt)
                 )
           (= 'sentinel (:data xx))
                (reset-d-next! (d-next nu-node) (d-sentinel xx))
                (reset-d-prev! (d-prev nu-node) xx)
                (reset-d-prev! (d-sentinel xx) nu-node)
                (reset-d-size! (d-size xx) (+ 1 (d-size x)))
          :else (if (> elt (:data xx))
                    (inserted-sorted (d-next xx) elt)
                    (do
                      (reset-d-prev! (d-prev nu-node) (d-next xx))
                      (reset-d-next! (d-next nu-node) (d-prev xx))
                      (reset-d-prev! (d-prev xx) (d-next nu-node))
                      (reset-d-next! (d-next xx) (d-prev nu-node))
                      (reset-d-size! (d-size xx) (+ 1 (d-size x))))
                )
    )
  )
