...
(defn insert-last "Insert an element into the back of a dlist."
  [xx elt]
  (let [nu-node (dnode (d-prev (d-sentinel xx)) elt (d-sentinel xx))]
  	(reset-d-prev! (d-sentinel xx) nu-node)
	(reset-d-next! (d-prev nu-node) nu-node)
	(swap-d-size! xx inc))
)

(defn find-location [node elt]
	(cond (identical? "sentinel" (d-data node)) nil
	(< elt (d-data node)) node
	:else (find-location (d-next node) elt))
)
(defn get-front [xx]
	(d-next (d-sentinel xx)))


(defn insert-sorted "Insert an element in sorted order"
  [xx elt]
  (if (= 0 (d-size xx)) (insert-front xx elt)
  (let [x (find-location (get-front xx) elt)
        nu-node (dnode (d-prev x) elt x)]
	(if (= x nil) (insert-last xx elt)
	        (do   (reset-d-next! (d-prev nu-node) nu-node)
		      (reset-d-prev! (d-next nu-node) nu-node)
		      (swap-d-size! xx inc)))))
)


(defn getIndex[node elt i]
(cond (= 'sentinel (d-data node)) nil
	(= elt (d-data node)) i
	:else (getIndex (d-next node) elt (inc i)))
)
...
I.e., if it was the first element, return 0; second, return 1..."
  [xx elt]
  (getIndex (get-front xx) elt 0))

(defn getIndBack [node elt i]
(cond (= 'sentinel (d-data node)) nil
	(= elt (d-data node)) i
	:else (getIndBack (d-prev node) elt (dec i)))
)
...
I.e., if it was the last element, return -1; penultimate, return -2..."
  [xx elt]
  (getIndBack (d-prev (d-sentinel xx)) elt -1))
