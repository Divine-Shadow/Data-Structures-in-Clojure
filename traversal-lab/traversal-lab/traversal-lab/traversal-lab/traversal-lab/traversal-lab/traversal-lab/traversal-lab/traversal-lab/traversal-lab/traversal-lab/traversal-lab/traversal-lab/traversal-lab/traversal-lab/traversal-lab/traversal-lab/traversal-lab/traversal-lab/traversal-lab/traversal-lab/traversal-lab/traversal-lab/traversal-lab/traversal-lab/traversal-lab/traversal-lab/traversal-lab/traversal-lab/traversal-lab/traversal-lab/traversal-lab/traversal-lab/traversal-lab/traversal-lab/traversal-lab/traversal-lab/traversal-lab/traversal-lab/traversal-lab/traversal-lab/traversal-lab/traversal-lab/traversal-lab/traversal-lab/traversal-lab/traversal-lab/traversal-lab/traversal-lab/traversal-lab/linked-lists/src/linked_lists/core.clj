...
(defn insert-at-end 
  ...
  the whole list."
  [elt xx] (if (empty? xx) (Cons.  elt nil)   (Cons. (:car xx) (insert-at-end elt (:cdr xx)))))

;; # Sorted Insert

(defn sorted-insert
  "Insert an element into a sorted list."
  [elt xx] ( cond  (empty? xx) (Cons.  elt nil))  (< elt (:car xx)) (Cons. elt xx) :else (Cons. (:car xx) (sorted-insert elt (:cdr xx))))
;; # Search
;;
...
(defn search 
  ...
  [elt xx] (cond (empty? xx) false (= elt (:car xx)) true :else (search elt (:cdr xx))))
...
(defn delete
  ...
  [elt xx] (cond (empty? xx) xx (= elt (:car xx)) (:cdr xx) :else (Cons. (:car xx) (delete elt (:cdr xx)))))

;; # Delete all

(defn delete-all 
  "Delete all copies of elt from xx."
  [elt xx] (cond (empty? xx) xx (= elt (:car xx)) (delete-all elt (:cdr xx)) :else (Cons. (:car xx) (delete-all (:cdr xx)))))
...
(defn efficient-delete
  ...
  *original* xx instead of a copy.  It is acceptable to prescan the
  list."
  [elt xx] (if (= (search elt xx) true) (delete elt xx) xx))
