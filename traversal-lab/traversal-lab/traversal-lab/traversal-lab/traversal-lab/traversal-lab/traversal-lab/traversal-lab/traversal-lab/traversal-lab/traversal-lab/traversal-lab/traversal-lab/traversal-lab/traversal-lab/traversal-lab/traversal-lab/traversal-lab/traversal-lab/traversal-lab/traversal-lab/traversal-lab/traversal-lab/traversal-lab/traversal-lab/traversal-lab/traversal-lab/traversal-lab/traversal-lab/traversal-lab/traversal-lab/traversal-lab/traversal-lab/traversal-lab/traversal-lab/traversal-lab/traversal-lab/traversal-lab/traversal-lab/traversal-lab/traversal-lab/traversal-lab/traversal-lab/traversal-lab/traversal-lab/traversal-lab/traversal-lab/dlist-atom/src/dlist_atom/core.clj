...
(defn add-first-item "add first item to a DList--no test" 
  [xx elt]
  (let [nu-node (dnode (d-sentinel xx) elt (d-sentinel xx))
        sent (d-sentinel xx)]
    (reset-d-next! sent nu-node)
    (reset-d-prev! sent nu-node)
    ))

(defn insert-front "Insert an element into the front of a dlist."
  [xx elt]
  (cond 
    (nil? xx) (insert-front (dlist) elt)
    (nil? (-> xx d-sentinel d-next)) (add-first-item xx elt)
    :else (let [nu-node (dnode (d-sentinel xx) elt (-> xx d-sentinel d-next))]
...
(defn insert-last "Insert an element into the back of a dlist."
  [xx elt]
  (cond 
    (nil? xx) (insert-last (dlist) elt) 
    (nil? (-> xx d-sentinel d-prev)) (add-first-item xx elt)
...
(defn insert "insert item before currNode in DList--no test"
  [elt currNode]
  (let [nu-node (dnode (-> currNode d-prev) elt currNode)]
    (reset-d-prev! currNode nu-node)
    (reset-d-next! (d-prev nu-node) nu-node))
  )
(defn add-first-item "add first item to a DList--no test" 
  [xx elt]
...
(defn insert-sorted "Insert an element in sorted order"
  ([xx elt]
  (cond 
    (nil? xx) (insert-sorted (dlist) elt)
    (nil? (-> xx d-sentinel d-next)) (add-first-item xx elt) 
    :else (do (insert-sorted (-> xx d-sentinel d-next) elt (d-sentinel xx))
    (reset-d-size! xx (inc  (d-size xx)))
    xx
    )))
  ([currNode elt sent]
  (cond 
    (or (identical? currNode sent) (< elt (d-data currNode))) (insert elt currNode)
    (or (= (d-data currNode) elt) (> elt (d-data currNode))) (insert-sorted (d-next currNode) elt sent)
    )))
...
I.e., if it was the first element, return 0; second, return 1..."
  ([xx elt]
  (index-forward (-> xx d-sentinel d-next) elt (d-sentinel xx) 0))
  ([currNode elt sent count]
    (cond
      (identical? currNode sent) nil
      (= (d-data currNode) elt) count
      :else (index-forward (d-next currNode) elt sent (inc count))
      )))
...
I.e., if it was the last element, return -1; penultimate, return -2..."
  ([xx elt]
  (index-backward (-> xx d-sentinel d-prev) elt (d-sentinel xx) -1))
  ([currNode elt sent count]
    (cond
      (identical? currNode sent) nil
      (= (d-data currNode) elt) count
      :else (index-backward (d-prev currNode) elt sent (dec count))
      )))

(defn list-to-dlist "Given a Clojure list, return a DList with the same content."
  ([xx]
    (list-to-dlist (dlist) xx))
  ([xx elts]
    (cond
      (empty? elts) xx
      (empty? (rest elts)) (insert-last xx (first elts))
      :else (list-to-dlist (insert-last xx (first elts)) (rest elts))
      )) 
  )
...
Uses sentinels, so it's very short."
  ([xx victim]
  (do
    (if (delete (-> xx d-sentinel d-next) victim (d-sentinel xx)) (reset-d-size! xx (dec (d-size xx))) nil)
    xx
    ))
  ([currNode victim sent]
    (cond
      (identical? currNode sent) nil
      (= (d-data currNode) victim) 
         (do
           (reset-d-next! (d-prev currNode) (d-next currNode))
           (reset-d-prev! (d-next currNode) (d-prev currNode)))
      :else (delete (d-next currNode)victim sent)
      )))
(defn flip-links "switch the previous link with the next link" 
  [currNode]
  (do (let [old-prev (d-prev currNode)]
    (reset-d-prev! currNode (d-next currNode))
    (reset-d-next! currNode old-prev))
     currNode)) 
(defn reverse "Reverse the doubly linked list in place.  No new DNode or DList records are created."
  ([xx]
    (do 
      (reverse (-> xx d-sentinel d-next) (d-sentinel xx))
       xx))
  ([currNode sent]
    (cond
      (identical? currNode sent) (flip-links currNode)
      :else (reverse (d-prev (flip-links currNode)) sent))) 
  )

(defn show-dlist-reverse-aux "NT---Run through the data of a Dlist, stopping when reaching the sentinel or
discovering an infintite loop.  The runner goes twice as fast through the list; if it reaches
node we know we have an infinite loop."
  [sen node runner]
  (cond (identical? sen node) '()
...
(defn pdl "return DList-atom list as standard clojure list--no test"
  ([xx] 
    (pdl (-> xx d-sentinel d-next) (d-sentinel xx))
    )
  ([curr sent]
    (cond 
      (identical? sent curr) '()
      :else (cons (d-data curr) (pdl (d-next curr) sent))
      ))
  )