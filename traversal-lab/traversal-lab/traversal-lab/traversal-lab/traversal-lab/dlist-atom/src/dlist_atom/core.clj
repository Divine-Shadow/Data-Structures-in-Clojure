...
(defn delete-aux [node value sentinel]
  (cond (identical? sentinel node) value
        (= (d-data node) value) (do (reset-d-prev! (d-next node)
                                                   (d-prev node))
                                    (reset-d-next! (d-prev node) (d-next node)))
        :else (delete-aux (d-next node) value sentinel)))
...
(defn rev-aux [sentinel node]
  (if (identical? sentinel node)
    (do (let [oldNext (d-next node)]
          (reset-d-next! node (d-prev node))
          (reset-d-prev! node oldNext)))
    (do (let [oldNext (d-next node)]
          (reset-d-next! node (d-prev node))
          (reset-d-prev! node oldNext)
          (rev-aux sentinel (d-next node))))))
...
(defn show-dlist-reverse "Like show-dlist, but returns the items in reverse order."
  [xx]
  (let [p xx]
    (do (reverse p) (let [e (show-dlist p)] (reverse p) e))))
